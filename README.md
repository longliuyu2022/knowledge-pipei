...
| 灵魂匹配 | 手动开始、在线配对、同频/互补偏好、兴趣筛选、取消、换一个、双方确认后直达聊天 |
...
用户已亲自完成知乎授权并确认登录成功，服务端也已建立知乎账号。回调与赛事登记一致，应用仍严格检查一次性 `state`、当前浏览器和原会话。官方 CLI 的本人凭证与最小内容读取已通过；创作、关注和收藏相关的五项 OAuth 用户接口尚未完成公网实测，具体范围见 [知乎接入记录](docs/ZHIHU_SETUP.md)。当前官方能力不包含点赞或浏览历史。
