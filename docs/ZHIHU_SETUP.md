...
入口的协议、授权、空数据、异常、并发撤销和响应隐私已经通过隔离自动化；其真实账号结果仍须由用户在正常授权会话中运行后记录，不能用 Mock 或 CLI 本人查询替代。
...
五项 OAuth 用户接口需要在已授权用户的会话中各读取至多一条。不从用户浏览器提取会话；本轮已交付正常登录后的最小检查入口，尚待用户实际运行这组公网验收；CLI 的本人凭证验收不等同于 OAuth 用户接口验收。
