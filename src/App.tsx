...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className={`sidebar ${mobileMenu ? 'sidebar-open' : ''}`}>
      ...
      <div className="sidebar-bottom"><button className="nav-item privacy-nav" onClick={() => open('settings')}><ShieldCheck size={18}/><span>数据与隐私</span></button><div className="sidebar-user"><Avatar name={data.profile?.input.name || data.user.name} seed={data.user.id} src={data.user.avatar} size={36}/><div><strong>{data.profile?.input.name || '好奇的朋友'}</strong><span>{data.user.provider === 'zhihu' ? '知乎已连接' : '从兴趣开始探索'}</span></div><button className="icon-button" aria-label="账号设置" onClick={() => open('settings')}><ChevronRight size={16}/></button></div></div>
    </aside>
