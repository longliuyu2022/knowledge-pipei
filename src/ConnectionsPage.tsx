...
export function ConnectionsPage({ actions, version }: { actions: PageActions; version: number }) {
  return <ConnectionsContent key={actions.data.user.id} actions={actions} version={version}/>;
}

function ConnectionsContent({ actions, version }: { actions: PageActions; version: number }) {
  const [tab, setTab] = useState<ConnectionTab>('saved');
  ...
  return <div className="connections-page">
    <PageTitle eyebrow="MEANINGFUL CONNECTIONS" title="让相遇，有下文" description="收藏值得认识的人，让共同的好奇心慢慢生长。"><button className="button secondary" onClick={() => actions.navigate('discover')}>发现更多伙伴<ArrowUpRight size={16}/></button></PageTitle>
    {!actions.data.profile && <div className="connection-notice connections-profile-notice"><UserRoundCheck size={20}/><div><strong>有了自己的知识人格，连接才有起点</strong><p>你可以先收藏感兴趣的伙伴，生成画像后再向真实参与者发出邀请。</p></div><button className="text-button" onClick={actions.onCreate}>生成我的画像<ArrowUpRight size={15}/></button></div>}
  ...
  return <div className="conversation-panel">
    ...
    <div className="conversation-messages" ref={viewport} tabIndex={0} aria-label={`与 ${invitation.person.name} 的消息记录`} onScroll={() => { const element = viewport.current; if (element) nearBottom.current = element.scrollHeight - element.scrollTop - element.clientHeight < 90; }}>
      {!conversation && loading ? <div className="conversation-initial-loading"><Spinner text="正在打开你们的对话…"/></div> : conversation && <>
        <div className="conversation-origin"><span><UserRoundCheck size={13}/>对话始于这条邀请</span><p>{conversation.invitation}</p></div>
