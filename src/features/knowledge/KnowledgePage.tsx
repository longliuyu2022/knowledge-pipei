...
export function KnowledgePage({ actions, version, onNavigate }: { actions: PageActions; version: number; onNavigate: (page: string) => void }) {
  ...
  return <div className="tz-feature" data-testid="knowledge-page">
    ...
    <div className="tz-tabs tz-page-tabs" role="group" aria-label="知识画像视图"><button className={tab === 'report' ? 'active' : ''} aria-pressed={tab === 'report'} onClick={() => setTab('report')}><BookOpen size={15}/>知识报告</button><button className={tab === 'portrait' ? 'active' : ''} aria-pressed={tab === 'portrait'} onClick={() => setTab('portrait')}><FileText size={15}/>画像与分享卡</button><button className={tab === 'suggestions' ? 'active' : ''} aria-pressed={tab === 'suggestions'} onClick={() => setTab('suggestions')}><ListChecks size={15}/>待确认知识</button></div>
